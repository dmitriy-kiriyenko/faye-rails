...
describe "Routing hooks" do
  ...
  let(:middleware) { Dummy::Application.middleware }

  describe 'middlware stack' do
    it "should contain two instance of Faye::Rails" do
      middleware.select { |m| m == FayeRails::Middleware }.count.should == 2
    end
  end
  ...
  describe "/faye_with_extension" do
    ...
    it "should be extended with MockExtension" do
      server = FayeRails.servers.select {|server| server.endpoint =~ /faye_with_extension/ }.first.server
      extensions = server.instance_variable_get :@extensions

      extensions.detect { |e|
        e.instance_of? Dummy::Application::MockExtension
      }.should be
    end
